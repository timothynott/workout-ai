...
1. Install Wrangler and authenticate:
   ...
   wrangler login
   ```
2. Create the Worker via `wrangler.toml` (generated by `@opennextjs/cloudflare` setup).
3. Set production secrets via the Cloudflare dashboard or Wrangler:
   ...
   ```
4. Add the following plain environment variables in `wrangler.toml` under `[vars]`:
